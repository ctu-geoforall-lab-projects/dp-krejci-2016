...
\paragraph{Management of connections} Module offers several flags for handling management of connections. In table\ref{tbl:connections} are described flags and theirs functions. In case of controlling several Hadoop clusters is suitable  approach to define its connection profiles  and switching between by setting the primary one by flag -a with parameter \textit{conn\_id} and \textit{driver}.


\subsubsection{Hadoop interaction}
The modules start with \textit{hdfs.*}(exclude \textit{hdfs.db.connection}) provide tools for transferring data between GRASS maps or filsystem and Hadoop distributed file system and its metadata. 

\subsubsection{Hive interaction}
