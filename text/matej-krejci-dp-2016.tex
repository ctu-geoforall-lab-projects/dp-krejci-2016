...
%		HiveQL."}
	...
	\newpage
	\chapter*{Experimental Framework}\stepcounter{chapter}\addcontentsline{toc}{chapter}{Experimental
		framework}
...
%	In first part has been introduced functionality of Esri package and its user defined functions. In addition %within the library is tool for serialisation JSON files.
...
%v.patch -e %input=italy_agg1_14@matt,italy_agg1_1@matt,italy_agg1_10@matt,italy_agg1_10@matt,italy_agg1_11@matt,italy_agg1_12@matt,italy_agg1_13@matt,italy_agg1_15 %@matt,italy_agg1_16@matt,italy_agg1_17@matt,italy_agg1_18@matt,italy_agg1_19@matt,italy_agg1_2@matt,italy_agg1_20@matt,italy_agg1_21@matt,italy_agg1_3@ %matt,italy_agg1_4@matt,italy_agg1_5@matt,italy_agg1_6@matt,italy_agg1_7@matt,italy_agg1_8@matt,italy_agg1_9@matt output=italy_agg1_fina
	...
	The main target of the work was design and implement processing workflow for interaction  between  Hadoop and GRASS GIS. Especially brings native access to distributed filesystem and data warehouse Hive within GRASS. This combination allows to use external libraries for parallel spatial processing. 
	
	As experiment within work has been developed GRASS Hadoop framework(GHF).
	The package includes several GRASS command line modules for interaction between Hadoop and Hive. The full pack of modules simplified and automated the processing workflow. Therefore conversion data to serialized format, provide transaction of data to distributed filesystem and load data to Hive data warehouse which supports two  spatial libraries.  The result of demanded high performance spatial analysis the GRASS Hadoop Framework provide transfer of results to local filesystem and conversion to the native GRASS vector map. 
...
206.8 ms
		\end{lstlisting}
	\end{footnotesize}	
		SELECT linkid,time,txpower-rxpower as a FROM %s.record  WHERE time >= '%s' AND time <= '%s'          ORDER by recordid;
...
\section{Attachment: installation of GRASS and GHF}
	...
	%%% ML: rozdeleni na windows a linux bych nedelal, na obou systemech se addons instaluji z pohledu uzivatele stejne, co je zatim neni pro praci podstatne
	...
	%%% ML: tady mas na mysli, ze se nainstaluji vsechny moduly jako jeden balicek (jako toolbox)???
		\begin{footnotesize}
			\begin{lstlisting}[style=python]
