...
%%% ML: v textu mluvis o hdfs.vector.import, tady o hdfs.in.vector, je potreba to sjednotit
%%% ML: k predchozi poznamce, prefix hdfs. je za me taky OK i kdyz deslo
%%% ML: nazvy modulu v teto tabulce mi prijdou OK
%%% ML: sladil bycj je ale pod jednen prefix, co tedy 'hd', tj. hd.db.connect, hd.hive.json.table a pod, kazdopadne by to mel byt jeden prefix!!!
	\begin{table}[!htbp]
		\centering
		\begin{scriptsize}
			\begin{tabular}{@{}|l|l||l|l|@{}}
...
%%% ML: chapter	
%%% ML: prectu jeste jednou, stejne jako uvod, az bude hotovo
	\section{Conclusion}
	
	The first part of the thesis introduced essentials features of Hadoop design and its
%%% ML: problematic
	...
	done. Followed by discussion dealing with the challenges connected to spatial
	processing using Map\-Reduce/HDFS and walk through particulars solutions of each
	library. The rest of first part is focused on Hadoop deployment on cluster using Google
	Cloud Platform.
	
%%% ML: experiment part zni divne (practical framework), mam pocit, ze to mas v textu i takto
